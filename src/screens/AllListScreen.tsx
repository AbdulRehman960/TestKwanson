...
const AllListScreen = ({ navigation }: any) => {
  ...
  const readData = async () => {
    ...
    let results = await ExecuteQuery(`SELECT * FROM tbl_kwanso ORDER BY time LIMIT ${limit}  OFFSET ${offset}`, [])
    let data = []
  ...
  const Item = useCallback(({ title }) => {
    return (
      <TouchableOpacity style={[styles.item,{backgroundColor:colors.alpha}]} onPress={() => { navigation.navigate("ViewListScreen", { DATA: title }) }}>
        <Text style={styles.title}>{"List Item"}</Text>
        <Image source={require('../assets/images/Next.png')} style={{ height: 18, width: 18 }} />
      </TouchableOpacity>
    );
  },
    [DATA],
